...
`Disconnected`. Using `$SSH_CLIENT` matters: it is observed by the server rather than reported by a
third party, so it cannot be wrong about what the firewall will actually see.
...
- **WireGuard or Tailscale**, with `:6443` allowed only from the tunnel interface or subnet. Best fit
  for workstation-native `kubectl`. **Verify API certificate SAN compatibility before depending on a
  new address** — a SAN change is a separate Kubespray-aligned operation, not part of a firewall PR.
